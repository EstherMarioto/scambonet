...
$sth = $pdo->prepare('select *from tbl_resposta r 
INNER JOIN tbl_usuario u on r.res_usu_codigo = u.usu_codigo
INNER JOIN tbl_oferta o on r res_ofe_codigo = o.ofe_codigo
INNER JOIN tbl_doacao d on r res_doa_codigo = d.doa_codigo
');
?>
<html lang="en">
...
<body>
  ...
  <div class="row">
    ...
    <div class="row">
      ...
      <?php
      $sth = $pdo->prepare('select *from tbl_resposta r 
              INNER JOIN tbl_oferta o on o.ofe_codigo = r.res_ofe_codigo
              INNER JOIN tbl_usuario u on u.usu_codigo = r.res_usu_codigo
              WHERE ofe_usu_codigo = :usuario');
      ...
      if ($sth->rowCount() > 0) :
        foreach ($sth as $res) :
          extract($res);
          $produto_oferecido = $ofe_pro_codigo;
          ?>
          <div class="container">
            <ul class="collection">
              <li class="collection-item avatar ">
                ...
                <form name="form1" method="post" action="updateCon.php">
                  <input name="ofe_codigo" type="hidden" value="<?= $pro_codigo; ?>"><br />
                  <input name="status" type="hidden" value=2><br />

                  <div class="row">
                    <a href="updateCon.php?ofe_codigo=<?= $ofe_codigo; ?>&statuss=2" class="btn cyan lighten-3"><span class=" white-text">Confirmar</span> </a>

                </form>

                <a class="btn cyan lighten-3" href="deletar.php?id='<?= $ofe_codigo ?> & idd=<?= $res_codigo ?>'"><span class=" white-text">Recusar</span></a>
                <div class="right">
                  <h5>
                    <?php
                        $sth = $pdo->prepare('
...
where o.ofe_codigo = :ofe_codigo');
      ...
      <?php
        endforeach;
      else : echo ' <center> <img src="../img/o.gif" class="circle responsive-img">
<p><h5>Nenhuma resposta no momento</h5></p>
</center> ';
      endif;
      ?>
      ...
      <?php
      $sth = $pdo->prepare('select *from tbl_resposta r 
              INNER JOIN tbl_doacao d on d.doa_codigo = r.res_doa_codigo
              INNER JOIN tbl_usuario u on u.usu_codigo = r.res_usu_codigo
              WHERE doa_usu_codigo = :usuario');
      $sth->bindValue(':usuario', $_SESSION['scambo']['id']);
      ...
      if ($sth->rowCount() > 0) :
        foreach ($sth as $res) :
          extract($res);
          $produto_oferecido = $doa_pro_codigo;
          ?>
          <div class="container">
            <ul class="collection">
              <li class="collection-item avatar ">
                ...
                <form name="form1" method="post" action="updateCon.php">
                  ...
                  <div class="row">
                    <a href="updateDo.php?doa_codigo=<?= $doa_codigo; ?>&statuss=2" class="btn cyan lighten-3"><span class=" white-text">Confirmar</span> </a>
                </form>
                <a class="btn cyan lighten-3" href="deleteDo.php?id='<?= $doa_codigo ?> & idd=<?= $res_codigo ?>'"><span class=" white-text">Recusar</span></a>
